...
#define G_SENSOR_VDD18 MT6325_POWER_LDO_VGP1
#define G_SENSOR_VDD28 MT6325_POWER_LDO_VIO28
/*----------------------------------------------------------------------------*/
...
static void LIS3DH_power(struct lis3dh_i2c_data *data, unsigned int on) 
{
#if 1
...
//	int err;
	//struct lis3dh_i2c_data *obj = container_of(obj->hw,struct lis3dh_i2c_data, hw);
	
/*	reg = regulator_get(data->client->dev,G_SENSOR_VDD28); // get pointer to regulator structure
        if (IS_ERR(reg)) {
                GTP_ERROR("regulator_get() failed!\n");
        }
	reg1 = regulator_get(data->client->dev,G_SENSOR_VDD18); // get pointer to regulator structure
        if (IS_ERR(reg)) {
                GTP_ERROR("regulator_get() failed!\n");
        }
*/
	if(data->hw->power_id != POWER_NONE_MACRO)		// have externel LDO
	{        
		GSE_LOG("power %s\n", on ? "on" : "off");
		...
		{
/*			err = regulator_set_voltage(reg,2800000,2800000);
			if (err)
         	       	     GSE_ERR("regulator_set_voltage() failed!\n");
			regulator_enable(reg);
			err = regulator_set_voltage(reg1,1800000,1800000);
			if (err)
                	     GSE_ERR("regulator_set_voltage() failed!\n");
			regulator_enable(reg);
...
static int LIS3DH_ReadData(struct i2c_client *client, s16 data[LIS3DH_AXES_NUM])
{
	...
	data[LIS3DH_AXIS_Z] = 0 -data[LIS3DH_AXIS_Z];
	return err;
}
/*----------------------------------------------------------------------------*/
/*
...
static int lis3dh_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	atomic_set(&obj->suspend, 0);
	GSE_ERR("%s: Hollen Into after i2c_set_clientdata\n", __func__); 
	LIS3DH_power(obj, 1);
	...
	GSE_ERR("%s: Hollen before LIS3DH_Init \n", __func__); 
	...
	GSE_ERR("%s: Hollen after LIS3DH_Init \n", __func__); 
...
static int  lis3dh_local_init(void)
{
   struct acc_hw *hw = lis3dh_get_cust_acc_hw();
	...
	GSE_ERR("%s: Hollen Into \n", __func__); 
	//LIS3DH_power(hw, 1);
	GSE_ERR("%s: after LIS3DH_power\n", __func__); 
		printk("============brucelee lis3dh_local_init======= init\n");
