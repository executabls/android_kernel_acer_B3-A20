...
int kdCISModulePowerOn(CAMERA_DUAL_CAMERA_SENSOR_ENUM SensorIdx, char *currSensorName, BOOL On, char* mode_name)
{
    ...
    if (On) {
		...
		else if (currSensorName && pinSetIdx == 0 && (0 == strcmp(currSensorName, SENSOR_DRVNAME_OV5670_MIPI_RAW)))
        {
            ...
            //First Power Pin low and Reset Pin Low
           else{ if (GPIO_CAMERA_INVALID != pinSet[pinSetIdx][IDX_PS_CMPDN]){
                ...
                if(mt_set_gpio_out(pinSet[1 - pinSetIdx][IDX_PS_CMPDN],pinSet[1 - pinSetIdx][IDX_PS_CMPDN+IDX_PS_ON])){PK_DBG("[CAMERA LENS] set gpio failed!! (CMPDN)\n");}
