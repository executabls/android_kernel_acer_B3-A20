...
ssize_t goodix_tool_write(struct file *filp, const char __user *buff, size_t len, loff_t *off)
{
	...
	else if(25 == cmd_head.wr)
	{
		GTP_INFO("setting the gesture");
		ret = copy_from_user(cmd_head.addr, &buff[CMD_HEAD_LENGTH], cmd_head.data_len);
	       if (ret)
	       {
	            GTP_ERROR("copy_from_user failed.");
	       }
		//u8 tmp_config[GTP_CONFIG_MIN_LENGTH + GTP_ADDR_LENGTH] ={GTP_REG_CONFIG_DATA >> 8,GTP_REG_CONFIG_DATA & 0xff};		
		cmd_head.data[0] = (GTP_REG_CONFIG_DATA >> 8);
		cmd_head.data[1] = GTP_REG_CONFIG_DATA & 0xff;		
	       if (tool_i2c_read(cmd_head.data,GTP_CONFIG_MIN_LENGTH + GTP_ADDR_LENGTH) <= 0)
              {
                	GTP_ERROR("[READ]Read data failed!");
                	return FAIL;
              }	
		   
		cmd_head.data[GTP_ADDR_LENGTH + GTP_REG_GESTURE_TYPE - GTP_REG_CONFIG_DATA] = cmd_head.addr[0];
		cmd_head.data[GTP_ADDR_LENGTH + GTP_REG_GESTURE_TIME- GTP_REG_CONFIG_DATA] = cmd_head.addr[1];
		u8 checksum = 0;
		int i;
		for(i = GTP_ADDR_LENGTH;i < (GTP_CONFIG_MIN_LENGTH);i++)
		{
			checksum += cmd_head.data[i];
			//GTP_INFO("config[%d]:%d",i-2,cmd_head.data[i]);
		}
		GTP_INFO("checksum:%d",checksum);
		cmd_head.data[GTP_CONFIG_MIN_LENGTH + GTP_ADDR_LENGTH - 2] = (~checksum) + 1;
		cmd_head.data[GTP_CONFIG_MIN_LENGTH + GTP_ADDR_LENGTH - 1] = 1;
		if(tool_i2c_write(cmd_head.data,GTP_CONFIG_MIN_LENGTH + GTP_ADDR_LENGTH) <= 0)
		{
			GTP_ERROR("[WRITE]Write data failed!");
            		return FAIL;
		}
		//GTP_INFO("save gesture success");
	}
#endif
    return len;
}
...
#if CUSTOM_GESTURE_WAKEUP
	else if(10 == cmd_head.wr)
	{
		GTP_INFO("get the gesture state");
		//u8 tmp_config[GTP_CONFIG_MIN_LENGTH + GTP_ADDR_LENGTH] ={GTP_REG_CONFIG_DATA >> 8,GTP_REG_CONFIG_DATA & 0xff};		
		//u8* tmp_config = kzalloc(GTP_CONFIG_MIN_LENGTH + GTP_ADDR_LENGTH, GFP_KERNEL);
		//tool_i2c_read(tmp_config,sizeof(tmp_config));
		cmd_head.data[0] = (GTP_REG_CONFIG_DATA >> 8);
		cmd_head.data[1] = GTP_REG_CONFIG_DATA & 0xff;
	       if (tool_i2c_read(cmd_head.data,GTP_CONFIG_MIN_LENGTH + GTP_ADDR_LENGTH) <= 0)
              {
		...
		page[0] = cmd_head.data[GTP_ADDR_LENGTH + GTP_REG_GESTURE_TYPE - GTP_REG_CONFIG_DATA];
		page[1] = cmd_head.data[GTP_ADDR_LENGTH + GTP_REG_GESTURE_TIME- GTP_REG_CONFIG_DATA];
		//kfree(tmp_config);
		return cmd_head.data_len;
	}
#endif
