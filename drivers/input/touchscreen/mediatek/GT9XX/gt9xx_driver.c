...
int write_proc(struct file *file, const char __user *buf, unsigned long count, void *data);
...
//自定义手势标志初始化
u8 gesture_flag = 0;
struct timer_list double_tap_timer;
u8 double_tap_flag = 0;
u8 scan_wait_time = 1;
u8 gesture_suspend_flag = 0;
...
#endif
#if CUSTOM_GESTURE_WAKEUP
static void double_tap_timer_handler(unsigned long arg)
{
	double_tap_flag = 0;
	GTP_INFO(" clear double tap flag");
}
#endif
static int tpd_i2c_detect(struct i2c_client *client, struct i2c_board_info *info)
{
...
/*
	...
	mProc_dir_entry->read_proc = read_proc;
	mProc_dir_entry->write_proc = write_proc;
	*/
	memset(mProcData, 0, sizeof(mProcData));
...
static s32 gtp_init_panel(struct i2c_client *client)
{
	...
	printk(KERN_ERR "GT910 init panel\n");
...
#endif
#if (GTP_GESTURE_WAKEUP || CUSTOM_GESTURE_WAKEUP)
...
#endif
	printk(KERN_ERR "GT910 probe OK!!!\n"); 
...
static void force_reset_guitar(void)
{
	...
	pmic_set_register_value(PMIC_RG_VGP1_VOSEL,0x7);
        pmic_set_register_value(PMIC_VGP1_LP_SEL,0x7);
        pmic_set_register_value(PMIC_RG_VGP1_EN,0);
	//regulator_set_voltage(PMIC_RG_VGP1_VOSEL, 2800,2800);
	//regulator_set_voltage(PMIC_VGP1_LP_SEL, 2800,2800);
	//regulator_enable(PMIC_RG_VGP1_EN); 
    #else
...
static int touch_event_handler(void *unused)
{
    ...
    do
    {
        ...
        tpd_flag = 0;
	printk(KERN_ERR "GT910 interrupt OK\n");
        TPD_DEBUG_SET_TIME;
        ...
        else
        {
		  ...
		  #if CUSTOM_GESTURE_WAKEUP //自定义手势宏开关
          
//don't need to report something, framework and app can deal everything.  so cancle this code.
#if 0
                    GTP_INFO("-->>hxh CUSTOM_GESTURE_WAKEUP BEFORE gesture_suspend_flag=%d\n.",gesture_suspend_flag);
			if(gesture_suspend_flag)//此时处于LCD 灭屏状态
			{
				
				GTP_INFO("--->>hxh mIsEnableDoubleTab:%d,max_num:%d,double_tap_flag:%d",mIsEnableDoubleTab,max_num,double_tap_flag);
				if((mIsEnablefivefinger)&&(max_num== 5))//五指触摸唤醒
				{
					gesture_suspend_flag = 0;
					//input_report_key(tpd->dev, KEY_POWER, 1);
		               // input_sync(tpd->dev);
		              // input_report_key(tpd->dev, KEY_POWER, 0);
		               //input_sync(tpd->dev);
				}
				if((mIsEnabletwofinger)&&(max_num== 2))//两指触摸唤醒
				{
				    gesture_suspend_flag = 0;
					//input_report_key(tpd->dev, KEY_POWER, 1);
		               // input_sync(tpd->dev);
				...
				if((mIsEnableDoubleTab)&&(max_num== 1))//双击唤醒自定义时间
				{
					...
					{						
						...
						input_report_key(tpd->dev, KEY_POWER, 1);
        		                    ...
        		                    input_sync(tpd->dev);
        		                    */
						if(timer_pending(&double_tap_timer))//时间间隔
						{
							del_timer_sync(&double_tap_timer);
						}						
						double_tap_flag = 0;
					}
					else
					{
						double_tap_flag = 1;
						init_timer(&double_tap_timer);
						//double_tap_timer.data = void;
						double_tap_timer.function = double_tap_timer_handler;
						double_tap_timer.expires = jiffies +  (20* HZ/100);
						add_timer(&double_tap_timer);												
					}
        ...
        pre_touch = touch_num;
	 #if CUSTOM_GESTURE_WAKEUP
		 if(touch_num > max_num || touch_num == 0)//五指触摸抬起唤醒后需要将max_num 清掉
		 {
		 	max_num = touch_num;
		 } 
	#endif
...
static void tpd_suspend(struct early_suspend *h)
{
	...
	{	
        	GTP_INFO("tpd_suspend  Gesture  WakeUp on.");
		return;
	}

#if 0
	u8 tmp[3] = {GTP_REG_GESTURE_TYPE >> 8, GTP_REG_GESTURE_TYPE & 0xff, 0};//0x8088
	gtp_i2c_read(i2c_client_point,tmp,sizeof(tmp));
	gesture_flag = tmp[2];

	if(mIsEnabletwofinger || mIsEnablefivefinger || mIsEnableDoubleTab)
	{	 
		gesture_suspend_flag = 1;
		tmp[0] = GTP_REG_GESTURE_TIME >> 8;
		tmp[1] = GTP_REG_GESTURE_TIME & 0xff;
		gtp_i2c_read(i2c_client_point,tmp,sizeof(tmp));//0x808B
		//scan_wait_time= tmp[2];	
		scan_wait_time= 1;
		return;
	}
#endif
   ...
   // ret = gtp_enter_sleep(i2c_client_point);
	#if ACER_GESTURE_WAKEUP
		if (!mIsEnableGestureWakeUp)//上层手势开关是否打开
		{ 
			// Gesture WakeUp off
			mt_eint_mask(CUST_EINT_TOUCH_PANEL_NUM);
			ret = gtp_enter_sleep(i2c_client_point);
		}
		else//开了手势唤醒，IC 既不休眠也不睡眠
		{
			// Gesture WakeUp on
			tpd_halt = 0;
		}
...
#if ACER_GESTURE_WAKEUP
	GTP_INFO("ACER_GESTURE_WAKEUP resume.");
	if (mIsEnableGestureWakeUp)
	{
		// Gesture WakeUp on
		return;
